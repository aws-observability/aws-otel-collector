...
if [[ $COLLECTOR_PATCH_VER == "0" ]]; then
    ...
    if echo $OPERATOR_TAGS | grep -q $COLLECTOR_TAG_WITHOUT_PATCH_VER; then
        ...
        if [[ $(echo "$OPERATOR_TAGS" | grep "\"name\": \"$COLLECTOR_TAG_WITHOUT_PATCH_VER" -c) == "1" ]]; then 
            ...
            NEW_OPERATOR_TAG=$(echo "$OPERATOR_TAGS" | grep "\"name\": \"$COLLECTOR_TAG_WITHOUT_PATCH_VER" -m 1 | cut -d ":" -f2 | sed 's|[",]||g')
