// Unless explicitly stated otherwise all files in this repository are licensed under the Apache-2.0 License.
// This product includes software developed at Datadog (https://www.datadoghq.com/).
// Copyright 2019-Present Datadog, Inc.

package datadogV2

import (
	"fmt"

	"github.com/DataDog/datadog-api-client-go/v2/api/datadog"
)

// DataRelationshipsTeamsDataItemsType Teams resource type.
type DataRelationshipsTeamsDataItemsType string

// List of DataRelationshipsTeamsDataItemsType.
const (
	DATARELATIONSHIPSTEAMSDATAITEMSTYPE_TEAMS DataRelationshipsTeamsDataItemsType = "teams"
)

var allowedDataRelationshipsTeamsDataItemsTypeEnumValues = []DataRelationshipsTeamsDataItemsType{
	DATARELATIONSHIPSTEAMSDATAITEMSTYPE_TEAMS,
}

// GetAllowedValues reeturns the list of possible values.
func (v *DataRelationshipsTeamsDataItemsType) GetAllowedValues() []DataRelationshipsTeamsDataItemsType {
	return allowedDataRelationshipsTeamsDataItemsTypeEnumValues
}

// UnmarshalJSON deserializes the given payload.
func (v *DataRelationshipsTeamsDataItemsType) UnmarshalJSON(src []byte) error {
	var value string
	err := datadog.Unmarshal(src, &value)
	if err != nil {
		return err
	}
	*v = DataRelationshipsTeamsDataItemsType(value)
	return nil
}

// NewDataRelationshipsTeamsDataItemsTypeFromValue returns a pointer to a valid DataRelationshipsTeamsDataItemsType
// for the value passed as argument, or an error if the value passed is not allowed by the enum.
func NewDataRelationshipsTeamsDataItemsTypeFromValue(v string) (*DataRelationshipsTeamsDataItemsType, error) {
	ev := DataRelationshipsTeamsDataItemsType(v)
	if ev.IsValid() {
		return &ev, nil
	}
	return nil, fmt.Errorf("invalid value '%v' for DataRelationshipsTeamsDataItemsType: valid values are %v", v, allowedDataRelationshipsTeamsDataItemsTypeEnumValues)
}

// IsValid return true if the value is valid for the enum, false otherwise.
func (v DataRelationshipsTeamsDataItemsType) IsValid() bool {
	for _, existing := range allowedDataRelationshipsTeamsDataItemsTypeEnumValues {
		if existing == v {
			return true
		}
	}
	return false
}

// Ptr returns reference to DataRelationshipsTeamsDataItemsType value.
func (v DataRelationshipsTeamsDataItemsType) Ptr() *DataRelationshipsTeamsDataItemsType {
	return &v
}
