// Unless explicitly stated otherwise all files in this repository are licensed under the Apache-2.0 License.
// This product includes software developed at Datadog (https://www.datadoghq.com/).
// Copyright 2019-Present Datadog, Inc.

package datadogV2

import (
	"fmt"

	"github.com/DataDog/datadog-api-client-go/v2/api/datadog"
)

// Urgency Specifies the level of urgency for a routing rule (low, high, or dynamic).
type Urgency string

// List of Urgency.
const (
	URGENCY_LOW     Urgency = "low"
	URGENCY_HIGH    Urgency = "high"
	URGENCY_DYNAMIC Urgency = "dynamic"
)

var allowedUrgencyEnumValues = []Urgency{
	URGENCY_LOW,
	URGENCY_HIGH,
	URGENCY_DYNAMIC,
}

// GetAllowedValues reeturns the list of possible values.
func (v *Urgency) GetAllowedValues() []Urgency {
	return allowedUrgencyEnumValues
}

// UnmarshalJSON deserializes the given payload.
func (v *Urgency) UnmarshalJSON(src []byte) error {
	var value string
	err := datadog.Unmarshal(src, &value)
	if err != nil {
		return err
	}
	*v = Urgency(value)
	return nil
}

// NewUrgencyFromValue returns a pointer to a valid Urgency
// for the value passed as argument, or an error if the value passed is not allowed by the enum.
func NewUrgencyFromValue(v string) (*Urgency, error) {
	ev := Urgency(v)
	if ev.IsValid() {
		return &ev, nil
	}
	return nil, fmt.Errorf("invalid value '%v' for Urgency: valid values are %v", v, allowedUrgencyEnumValues)
}

// IsValid return true if the value is valid for the enum, false otherwise.
func (v Urgency) IsValid() bool {
	for _, existing := range allowedUrgencyEnumValues {
		if existing == v {
			return true
		}
	}
	return false
}

// Ptr returns reference to Urgency value.
func (v Urgency) Ptr() *Urgency {
	return &v
}
