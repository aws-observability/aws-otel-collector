// Unless explicitly stated otherwise all files in this repository are licensed under the Apache-2.0 License.
// This product includes software developed at Datadog (https://www.datadoghq.com/).
// Copyright 2019-Present Datadog, Inc.

package datadogV2

import (
	"fmt"

	"github.com/DataDog/datadog-api-client-go/v2/api/datadog"
)

// CreatePageRequestDataType The type of resource used when creating an On-Call Page.
type CreatePageRequestDataType string

// List of CreatePageRequestDataType.
const (
	CREATEPAGEREQUESTDATATYPE_PAGES CreatePageRequestDataType = "pages"
)

var allowedCreatePageRequestDataTypeEnumValues = []CreatePageRequestDataType{
	CREATEPAGEREQUESTDATATYPE_PAGES,
}

// GetAllowedValues reeturns the list of possible values.
func (v *CreatePageRequestDataType) GetAllowedValues() []CreatePageRequestDataType {
	return allowedCreatePageRequestDataTypeEnumValues
}

// UnmarshalJSON deserializes the given payload.
func (v *CreatePageRequestDataType) UnmarshalJSON(src []byte) error {
	var value string
	err := datadog.Unmarshal(src, &value)
	if err != nil {
		return err
	}
	*v = CreatePageRequestDataType(value)
	return nil
}

// NewCreatePageRequestDataTypeFromValue returns a pointer to a valid CreatePageRequestDataType
// for the value passed as argument, or an error if the value passed is not allowed by the enum.
func NewCreatePageRequestDataTypeFromValue(v string) (*CreatePageRequestDataType, error) {
	ev := CreatePageRequestDataType(v)
	if ev.IsValid() {
		return &ev, nil
	}
	return nil, fmt.Errorf("invalid value '%v' for CreatePageRequestDataType: valid values are %v", v, allowedCreatePageRequestDataTypeEnumValues)
}

// IsValid return true if the value is valid for the enum, false otherwise.
func (v CreatePageRequestDataType) IsValid() bool {
	for _, existing := range allowedCreatePageRequestDataTypeEnumValues {
		if existing == v {
			return true
		}
	}
	return false
}

// Ptr returns reference to CreatePageRequestDataType value.
func (v CreatePageRequestDataType) Ptr() *CreatePageRequestDataType {
	return &v
}
